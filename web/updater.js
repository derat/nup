...
function Updater() {
  ...
  this.queuedPlayReports = this.readObject(
    ...
    [],
  ).concat(this.readObject(Updater.IN_PROGRESS_PLAY_REPORTS_KEY, []));
  ...
  var oldInProgress = this.readObject(
...
Updater.prototype.writeState = function() {
  localStorage[Updater.QUEUED_PLAY_REPORTS_KEY] = JSON.stringify(
    this.queuedPlayReports,
  );
  localStorage[Updater.QUEUED_RATINGS_AND_TAGS_KEY] = JSON.stringify(
    this.queuedRatingsAndTags,
  );
  localStorage[Updater.IN_PROGRESS_PLAY_REPORTS_KEY] = JSON.stringify(
    this.inProgressPlayReports,
  );
  localStorage[Updater.IN_PROGRESS_RATINGS_AND_TAGS_KEY] = JSON.stringify(
    this.inProgressRatingsAndTags,
  );
};
...
Updater.prototype.removePlayReportFromArray = function(
  songId,
  startTime,
  list,
) {
...
Updater.prototype.addRatingAndTagsToObject = function(
  songId,
  rating,
  tags,
  obj,
